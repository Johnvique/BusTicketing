...
class PaymentsController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('id', 	'boking_id', 	'date', 	'transaction_no', 	'status');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text = $this->search;
			$db->orWhere('id',"%$text%",'LIKE');
			$db->orWhere('boking_id',"%$text%",'LIKE');
			$db->orWhere('date',"%$text%",'LIKE');
			$db->orWhere('transaction_no',"%$text%",'LIKE');
		...
		$this->view->page_title ="Payments";
		$this->view->render('payments/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'payments/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if(!empty($record)){
			$this->view->page_title ="View  Payments";
			$this->view->render('payments/view.php' , $record ,'main_layout.php');
		}
		else{
			if($db->getLastError()){
				$this->view->page_error = $db->getLastError();
	...
	function add(){
		if(is_post_request()){
			...
			if( empty($this->view->page_error) ){
				...
				if(!empty($rec_id)){
					set_flash_msg("here is your record",'success');
					redirect_to_page("payments/view/$rec_id");
					return;
				}
				else{
					if($db->getLastError()){
		...
		$this->view->page_title ="Add New Payments";
		$this->view->render('payments/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id=null){
		...
		if(is_post_request()){
			...
			if(isset($modeldata['boking_id'])){
				$db->where('boking_id',$modeldata['boking_id'])->where('id',$rec_id,'!=');
				if($db->has('payments')){
					$this->view->page_error[] = $modeldata['boking_id']."Already exist!";
				}
			} 
			if(empty($this->view->page_error)){
				$db->where('id' , $rec_id);
		...
		$this->view->page_title ="Edit  Payments";
		if(!empty($data)){
			$this->view->render('payments/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
				$this->view->page_error[] = $db->getLastError();
